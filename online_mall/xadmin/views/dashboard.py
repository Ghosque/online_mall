...
class UserWidgetAdmin(object):
    ...
    list_display = ('widget_type', 'page_id', 'buyer')
    list_filter = ['buyer', 'widget_type', 'page_id']
...
class Dashboard(CommAdminView):
    ...
    def get_context(self):
        new_context = {
            ...
            "?buyer=%s&page_id=%s&_redirect=%s" % (self.user.id, self.get_page_id(), urlquote(self.request.get_full_path()))
        }
